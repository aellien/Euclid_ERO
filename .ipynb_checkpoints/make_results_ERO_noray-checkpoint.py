...
import tracemalloc
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
def selection_error(atom_in_list, atom_out_list, M, percent, xs, ys, flux_lim, mscsedl, write_plots = False, nfp = None, name = None):
    '''Computation of classification error on flux.
    '''

    # deterministic image for plot normalization
    if write_plots == True:
        im_m = np.zeros((xs, ys))
        for (image, det_err_image, x_min, y_min, x_max, y_max, xco, yco, lvlo) in atom_in_list:
    ...
    for i, (s, r) in enumerate(zip(size_sample, replace_sample)):
        ...
        if write_plots == True:
            ...
            plt.savefig('/home/aellien/Euclid_ERO/wavelets/out7/bootstrap/%s%03d.png'%(nfp.split('/')[-1], i))
            plt.close('all')
    ...
    if write_plots == True:
        ...
        plt.savefig('/home/aellien/Euclid_ERO/wavelets/out7/bootstrap/%s%s.png'%(nfp.split('/')[-1], name))
        plt.close('all')
        #plt.show()
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
def synthesis_bcgwavsizesep_with_masks( nfp, lvl_sep, lvl_sep_max, lvl_sep_bcg, size_sep, size_sep_pix, xs, ys, n_levels, mscstar, mscell, mscbcg, mscsedl, msat, R, rc_pix, N_err, per_err, flux_lim, kurt_filt = True, plot_vignet = False, write_fits = True, measure_PR = False, plot_boot = False ):
    ...
    if measure_PR == True:
        ...
        F_ICL_m, F_ICL_low, F_ICL_up, FICL_det_err, low_FICL_det_err, up_FICL_det_err, out_sed_icl =  selection_error(tot_icl_al, tot_unclass_al+tot_gal_al, M = N_err, percent = per_err, xs = xs, ys = ys, flux_lim = flux_lim, mscsedl = mscsedl, write_plots = plot_boot, nfp = nfp)
